...
def norm_based_ordering(pixels, centerRgb, kernelSize):  # distance fonksiyonunu kullanmadan euclideanı bul
    ...
    for i in range(len(pixels)):
        if pixels[i] == None:
            ...
            euc = math.sqrt(abs(pow((centerRgb[0] - pixels[i][0]), 2) + pow((centerRgb[1] - pixels[i][1]), 2) + pow(
                (centerRgb[2] - pixels[i][2]), 2)))
        EucArr[i] = euc
        dic[euc] = pixels[i]
...
def scalar_median_filter(image, kernelSize, f):  # marginal
    ...
    for i in range(width):
        ...
        for j in range(height):
            ...
            arr_Green.sort()
            arr_Blue.sort()
            newRed, newGreen, newBlue = arr_Red[len(arr_Red) // 2], arr_Green[len(arr_Green) // 2], arr_Blue[
                len(arr_Blue) // 2]
