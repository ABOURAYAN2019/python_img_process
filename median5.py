...
if (os.path.exists("OriginalImages")):
    ...
    print(
        "No directory names 'OriginalImages' found. You must have a directory named 'OriginalImages' in your working directory")
...
def buildMasterArray():
    for count in range(1, numberOfImages + 1):
        try:
            ...
            print("Image " + str(
                count) + ".png not found in directory 'OriginalImages'. Your images must be named '#.png', starting with 1")
            return -1
        ...
        for x in range(0, width):
            for rgb in range(0, 3):
                if (numberOfImages % 2) is 1:
                    ...
                    master[y, x, rgb, int(numberOfImages / 2) + 1])) / 2
